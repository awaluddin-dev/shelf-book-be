...
export class RefreshTokenController {
  ...
  @Post('refresh')
  @ApiOperation({ summary: 'Refresh an expired access token using a refresh token' })
  @ApiResponse({ status: 200, description: 'Token successfully refreshed', type: RefreshResponseDto })
  @ApiResponse({ status: 401, description: 'Unauthorized or invalid refresh token' , schema: { example: { statusCode: 401, message: 'Unauthorized', error: 'Unauthorized' } }})
