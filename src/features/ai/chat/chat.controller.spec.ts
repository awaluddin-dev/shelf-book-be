...
describe('ChatController', () => {
  ...
  describe('chat', () => {
    ...
    it('should simulate SSE if response is application/json', async () => {
      ...
      expect(reply.raw.write).toHaveBeenCalledWith(`data: ${JSON.stringify({ choices: [{ delta: { content: 'test content' } }] })}\n\n`);
    ...
    it('should simulate SSE if response is application/json with delta content', async () => {
      ...
      chatService.streamChat.mockResolvedValue({
        body: {},
        headers,
        json: async () => ({ choices: [{ delta: { content: 'test content 2' } }] })
      } as any);
      ...
      await controller.chat(dto, reply);
      expect(reply.raw.write).toHaveBeenCalledWith(expect.stringContaining('test content 2'));
    });
