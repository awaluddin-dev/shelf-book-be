...
export class GithubService {
  async getContributions(username: string) {
    ...
    if (!token) {
      throw new HttpException('GitHub token is missing in environment', HttpStatus.INTERNAL_SERVER_ERROR);
    }
    ...
    try {
      ...
      const calendarResult: any[] = [];
      const monthlyAggregates: Record<string, { commits: number, pullRequests: number, issues: number }> = {};
      ...
      for (const week of calendar.weeks) {
        ...
        for (const day of week.contributionDays) {
          ...
          if (!monthlyAggregates[monthKey]) {
            monthlyAggregates[monthKey] = { commits: 0, pullRequests: 0, issues: 0 };
          }
      ...
      const timelineResult = Object.entries(monthlyAggregates).map(([month, data]) => ({
        month,
      ...
      const prRepos = contributionsCollection?.pullRequestContributionsByRepository || [];
      ...
      const repoResult = Object.entries(repoMap)
        .map(([name, data]) => ({ name, commits: data.commits, pullRequests: data.pullRequests }))
      ...
      throw new HttpException('Failed to fetch GitHub contributions', HttpStatus.BAD_REQUEST);
    }
  }
}
